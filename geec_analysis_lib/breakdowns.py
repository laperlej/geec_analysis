import geec_analysis_lib.hierarchical_clustering as hc
...
class Breakdown:
    ...
    def make_breakdown(matrix, metadata, breakdown_category, restriction_categories, tag, linkage_method):
        ...
        sub_categories = Categories.make_categories(metadata,
                                                    tag.get_file_names(),
                                                    restriction_categories)
        cluster_maker = hc.HierarchicalClustering(sub_matrix, linkage_method)
    ...
    def make_breakdown_of_all(matrix, categories, restriction_categories, linkage_method):
        cluster_maker = hc.HierarchicalClustering(matrix, linkage_method)
...
class SubBreakdown:
    ...
    def make_sub_breakdown(matrix, category, cluster_maker):
        nb_clusters, flat_clusters, linkage_matrix = cluster_maker.run(len(category))
        cat_flat_clusters = category.make_flat_clusters(matrix.get_file_names())

        # The data sources needs to be align
        # In our case we use the matrix order
        return SubBreakdown(adjusted_rand_score(list(flat_clusters), cat_flat_clusters),
                            len(category))
