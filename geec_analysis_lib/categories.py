class Categories(dict):
    def __init__(self, *args):
        super(Categories, self).__init__(*args)

    def obtain_file_names_with_tag_name(self, tag_name):
        return set(file_name for ca in self.itervalues() 
    ...
    def obtain_sorted_tags(self, category_name, **kwargs):
        return self[category_name].obtain_sorted_tags(**kwargs)
...
class Category:
    ...
    def obtain_sorted_tags(self, **kwargs):
        tags = list(self.tags.itervalues())
        tags.sort(**kwargs)
        return tags

    def make_flat_clusters(self, file_names):
        fn_flc = {}
        for idx, tag in enumerate(self.tags.itervalues()):
            for file_name in tag.get_file_names():
                fn_flc[file_name] = idx

        return [fn_flc[fn] for fn in file_names]
...
class Tags(dict):
    def __init__(self, *args):
        super(Tags, self).__init__(*args)

    def obtain_size_of_tags(self):
        return [(k, len(tag)) for k, tag in self.iteritems()]
    ...
    def obtain_file_names_tag_names(self):
        return dict([file_names_tag_names for tag in self.itervalues() for file_names_tag_names in tag.obtain_file_names_tag_names()])

    @staticmethod
    def make_tags(tags_file_names):
        return Tags({tag: Tag(tag, file_names) for tag, file_names in tags_file_names.iteritems()})
