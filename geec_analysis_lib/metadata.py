...
class Metadata:
    ...
    def _find(self, pattern, is_inverted):
        return [file_name
                for file_name, dataset in self.metadata['datasets'].iteritems()
                if any([str(data).find(pattern) >= 0
                        for data in dataset.itervalues()]) != is_inverted]
    ...
    def list_categories(self, file_names):
        ...
        for file_name in file_names:
            if file_name in datasets:
                ...
                categories_name.update(dataset.iterkeys())
...
class _Metadata(dict):
    ...
    def _extract_generic_keys(self, other={}):
        fusioned_keys = set(self.iterkeys()).union(other.iterkeys())
    ...
    def convert_export_format(self):
        datadict = {k:v
                    ...
                    if k != 'datasets'}
        datadict['datasets'] = self['datasets'].values()
        return datadict
    ...
    def make_metadata(raw_metadata):
        tuples = [(k, v)
                  for k, v in raw_metadata.iteritems()
                  if k != 'datasets']
...
class _Datasets(dict):
    ...
    def make_datasets(raw_datasets):
        return _Datasets([(k, _Dataset(v))
                          for k, v in raw_datasets.iteritems()])
