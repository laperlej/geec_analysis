...
class FirstPageComposer(composer.Composer):
    def __init__(self, annotation, title, ordered_group_names):
        self.annotation = annotation
        self.title = title
        self.ordered_group_names = ordered_group_names

    def run(self, left_lower_page, right_lower_page):
        linkage_matrix = self.annotation.get_linkage_matrix()
        ordered_matrix = self.annotation.get_ordered_matrix()

        threshold = self.annotation.get_nb_clusters()
        ...
        im = axmatrix.matshow(ordered_matrix.get_matrix(), interpolation='nearest', cmap='RdBu_r', origin='upper', vmin=-1, vmax=1)
        axmatrix.set_xticks([])
        axmatrix.set_yticks([])

        # User matrix line
        users_headers = self.annotation.obtain_file_names_with_tag_name(composer.USER)
        ...
        plt.plot([1 for useless in xrange(len(users_line_pos))], users_line_pos, 'ro', markersize=matplotlib.rcParams['font.size']/2., color=composer.LegendMaker.USER_COLOR)
        pylab.xlim([0.5, 1.5])
        ...
        # cluster id
        ivl = [int_label_to_letter_label(idx) for idx in Z['ivl']]

        axcluster = fig.add_axes([0.102,0.1,0.005,0.8])
        axcluster.axis('off')
        self._align_cluster_labels(axcluster, ivl, scaling)
        axcluster.set_xticks([])
        axcluster.set_yticks([])

        # description
        axdescription = fig.add_axes([0.11,0.015,0.4857142857,0.075])
        axdescription.text(0, 1, self.annotation.description, va='top', ha='left', clip_on=True, fontsize='large')
    ...
    def _draw_aligned_piecharts(self, fig):
        for column, group in enumerate(self.ordered_group_names[:3]):
            piecharts_builder = composer.PieChartsBuilder()
    ...
    def _draw_piecharts_lines(self, fig, piecharts, column):
        ...
        if nb_piecharts <= 10:
            line_a = reversed(xrange(nb_piecharts))
            line_b = xrange(0)
            stretch = 10 / float(nb_piecharts)
        elif nb_piecharts <= 20:
            side = nb_piecharts % 2 # 0 or 1
            line_a = reversed(xrange(1 - side, nb_piecharts, 2))
            line_b = reversed(xrange(0 + side, nb_piecharts, 2))
    ...
    def _draw_piecharts_line(self, fig, piecharts, cluster_ids, axes, pos_inc):
        for pos, cluster_id in enumerate(cluster_ids):
            axpie = fig.add_axes([axes[0], axes[1] + (pos * pos_inc), axes[2], axes[3]])
    ...
    def _align_cluster_labels(self, ax, ivl, scaling):
        ...
        # compile text
        for cluster_id in xrange(1, len(clusters) + 1):
