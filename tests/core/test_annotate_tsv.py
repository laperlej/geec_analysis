...
from geec_analysis_lib.annotate_tsv import AnnotateTsv
...
TSV_FILE = u'''file_name\tcluster(3)\tone\ttwo\tthree\tsix
...
class TestAnnotateTsv(unittest.TestCase):
    ...
    def test_make_filter(self):
        names = ['A', 'C']
        tsv_filter = self.annotate_tsv.make_filter('cluster(3)', lambda x: x in names)
        self.assertEqual(tsv_filter, [0, 3, 4, 5])
